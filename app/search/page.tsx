  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] font-sans">
      <div className="max-w-6xl mx-auto p-6">
        ...
        {filteredProductsList(filteredBooks, q)}
      </div>
    </div>
  );
}
  ...
  if (filteredBooks.length === 0) {
    return (
      <div className="bg-white p-12 rounded-[30px] text-center shadow-sm border border-slate-100">
        <p className="text-slate-400 text-lg">Ups! Buku <b>"{q}"</b> tidak ditemukan.</p>
      </div>
    );
  }
      ...
      {filteredBooks.map((book) => (
        ...
        >
          ...
          <div className="space-y-1">
            <p className="text-slate-800 font-bold leading-snug line-clamp-2 h-10">{book.title}</p>
