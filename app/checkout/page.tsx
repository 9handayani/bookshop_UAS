...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        ...
        // Jika user akses checkout tapi belum pilih barang di keranjang
        router.replace("/cart");
      }
    }
  ...
  const discount = subtotal * 0.05; // Diskon 5%
  ...
  const handlePayment = async () => {
    ...
    try {
      const payload = {
        ...
        // Gabungkan judul buku dan jumlahnya untuk database
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders`, {
        ...
        headers: {
          ...
          // Jika Laravel kamu pakai Sanctum, tambahkan ini:
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f5] p-8 flex justify-center font-sans text-slate-800">
      <div className="w-full max-w-7xl">
        <button 
          onClick={() => router.back()} 
          className="mb-6 flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-bold"
        >
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-[2] bg-white p-10 rounded-[30px] shadow-sm space-y-6 border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input type="text" placeholder="Nama Lengkap" className="p-4 bg-slate-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              <input type="text" placeholder="Nomor HP" className="p-4 bg-slate-50 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />
            </div>
            ...
            <div className="bg-white p-8 rounded-[30px] shadow-sm sticky top-8 border border-slate-100">
              ...
              <div className="space-y-4 mb-6">
                {checkoutItems.map((item) => {
                  // ✅ DISINKRONKAN DENGAN CartContext (MENGGUNAKAN item.img)
              ...
              <div className="space-y-3 pt-4 border-t border-dashed text-sm">
                ...
                <div className="flex justify-between text-red-500 font-bold"><span>Diskon (5%)</span><span>-Rp {discount.toLocaleString("id-ID")}</span></div>
              ...
              <button 
                ...
                className="mt-8 w-full bg-indigo-600 disabled:bg-slate-300 text-white py-4 rounded-2xl font-bold text-lg shadow-lg"
              >
